...
export async function buildCanvaExport(projectId: string, options: ExportOptions = {}) {
  ...
  if (project.creative_brief?.trim()) fs.writeFileSync(path.join(infoDir, 'creative-brief.txt'), project.creative_brief);
  ...
  const manifest = { exportVersion: 1, project: { title: project.title, projectType: project.project_type, creativeBrief: project.creative_brief, visualDirection: project.visual_style, aspectRatio: project.aspect_ratio, storyboardApproach: project.storyboard_approach, publishingTargets: JSON.parse(project.publishing_targets || '[]'), primaryVisualFormat:project.primary_visual_format }, selectedConcept: concept, visualIdentity: { style: identity.style.description, characters: identity.characters.map(item => ({ id: item.id, name: item.name, description: item.description })), locations: identity.locations.map(item => ({ id: item.id, name: item.name, description: item.description })) }, storyboard: { shots: summary.shots }, platformArtwork: artworkPaths, shots: shotData.map(({ shot, characterNames, location, image, status }) => ({ order: shot.position, title: shot.title, timing: { start: shot.start_seconds, end: shot.end_seconds, duration: shot.duration_seconds ?? (shot.start_seconds != null && shot.end_seconds != null ? shot.end_seconds - shot.start_seconds : null) }, activeImage: image, approvalStatus: shot.approval_status, status, characterNames, characterIds: JSON.parse(shot.character_ids || '[]'), location: location ? { id: location.id, name: location.name } : null, section: shot.section, description: shot.description, action: shot.action, shotType: shot.shot_type, camera: shot.camera, mood: shot.mood })), referenceAssets };

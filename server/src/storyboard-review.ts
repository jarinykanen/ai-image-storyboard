...
export async function reviewStoryboard(input: { project: { title: string; visual_style: string; lyrics: string; suno_description?: string | null }; plan: StoryboardPlan | null; identity: VisualIdentity; concept?: VisualConcept | null; shots: ReviewShot[] }) {
  const response = await getTextClient().responses.create({ model: 'gpt-5.6-terra', input: buildStoryboardReviewPrompt(input) });
  const result = ResultSchema.parse(parse(response.output_text));

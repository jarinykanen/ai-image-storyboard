...
export type ImageAsset = { id:string; projectId:string; ownerType:AssetOwnerType; ownerId:string; source:AssetSource; url:string; originalFilename:string|null; mimeType:string; fileSize:number; version:number; active:boolean; provider:string|null; model:string|null; quality:string|null; resolution:string|null; createdAt:string };
...
function asAsset(row: any): ImageAsset { return { id:row.id, projectId:row.project_id, ownerType:row.owner_type, ownerId:row.owner_id, source:row.source, url:assetUrl(row.id), originalFilename:row.original_filename ?? null, mimeType:row.mime_type, fileSize:row.file_size, version:row.version, active:Boolean(row.active), provider:row.provider ?? null, model:row.model ?? null, quality:row.quality ?? null, resolution:row.resolution ?? null, createdAt:row.created_at }; }
...
function addAsset(input: { projectId:string; ownerType:AssetOwnerType; ownerId:string; source:AssetSource; data:Buffer; mimeType:string; originalFilename?:string; provider?:string; model?:string; quality?:string; resolution?:string }) {
  ...
  try { db.transaction(() => { db.prepare('UPDATE image_assets SET active=0 WHERE owner_type=? AND owner_id=?').run(input.ownerType, input.ownerId); db.prepare(`INSERT INTO image_assets (id,project_id,owner_type,owner_id,source,storage_path,original_filename,mime_type,file_size,version,active,provider,model,quality,resolution,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id,input.projectId,input.ownerType,input.ownerId,input.source,relativePath,input.originalFilename ? safeName(input.originalFilename) : null,input.mimeType,input.data.length,version,1,input.provider ?? null,input.model ?? null,input.quality ?? null,input.resolution ?? null,new Date().toISOString()); })(); }

...
function asAsset(row: any): ImageAsset { return { id:row.id, projectId:row.project_id, conceptId:row.concept_id, ownerType:row.owner_type, ownerId:row.owner_id, source:row.source, url:assetUrl(row.id), originalFilename:row.original_filename ?? null, mimeType:row.mime_type, fileSize:row.file_size, version:row.version, active:Boolean(row.active), provider:row.provider ?? null, model:row.model ?? null, quality:row.quality ?? null, resolution:row.resolution ?? null, tier:row.source === 'uploaded' ? null : row.tier ?? (row.quality === 'best' ? 'FINAL' : row.quality === 'draft' ? 'DRAFT' : 'STANDARD'), stale:Boolean(row.stale), createdAt:row.created_at }; }
...
function addAsset(input: { projectId:string; ownerType:AssetOwnerType; ownerId:string; source:AssetSource; data:Buffer; mimeType:string; originalFilename?:string; provider?:string; model?:string; quality?:string; resolution?:string; tier?:string }) {
  ...
  const conceptId = input.ownerType==='concept' ? input.ownerId
    : input.ownerType==='reference' ? (db.prepare('SELECT concept_id FROM visual_references WHERE id=?').get(input.ownerId) as any)?.concept_id
    : input.ownerType==='shot' ? (db.prepare('SELECT concept_id FROM shots WHERE id=?').get(input.ownerId) as any)?.concept_id
    : input.ownerType==='artwork' ? (db.prepare('SELECT concept_id FROM project_artwork WHERE id=?').get(input.ownerId) as any)?.concept_id
    : input.ownerId;
  if(!conceptId) throw new Error('The image does not belong to a visual concept.');
  try { db.transaction(() => { db.prepare('UPDATE image_assets SET active=0 WHERE owner_type=? AND owner_id=?').run(input.ownerType, input.ownerId); db.prepare(`INSERT INTO image_assets (id,project_id,concept_id,owner_type,owner_id,source,storage_path,original_filename,mime_type,file_size,version,active,provider,model,quality,resolution,tier,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id,input.projectId,conceptId,input.ownerType,input.ownerId,input.source,relativePath,input.originalFilename ? safeName(input.originalFilename) : null,input.mimeType,input.data.length,version,1,input.provider ?? null,input.model ?? null,input.quality ?? null,input.resolution ?? null,input.source === 'uploaded' ? null : input.tier ?? null,new Date().toISOString()); })(); }

import { z } from 'zod';
import { getTextClient } from './providers.js';
...
type Project = { title: string; lyrics: string; suno_description?: string | null; visual_style: string; aspect_ratio: string; duration_seconds?: number | null };
...
export async function createStoryboard(input: { project: Project & { storyboard_approach?: string }; shotCount: number; visualIdentity: VisualIdentity; selectedConcept?: VisualConcept | null }) {
  const approach = (input.project.storyboard_approach || 'mixed') as StoryboardApproach;
  const response = await getTextClient().responses.create({ model: 'gpt-5.6-terra', input: buildStoryboardPrompt({ project: input.project, identity: input.visualIdentity, concept: input.selectedConcept, shotCount: input.shotCount, approach }) });
  const data = StoryboardSchema.parse(parse(response.output_text));
...
export async function regenerateStoryboardShot(input: { project: Project; visualIdentity: VisualIdentity; selectedConcept?: VisualConcept | null; plan: StoryboardPlan; previous?: StoryboardShotContent; current: StoryboardShotContent; next?: StoryboardShotContent }) {
  const response = await getTextClient().responses.create({ model: 'gpt-5.6-terra', input: buildShotRegenerationPrompt({ project: input.project, identity: input.visualIdentity, concept: input.selectedConcept, plan: input.plan, previous: input.previous, current: input.current, next: input.next }) });
  const content = StoryboardShotSchema.omit({ startTime: true, endTime: true }).parse(parse(response.output_text));

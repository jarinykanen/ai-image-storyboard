import { Button, Checkbox, Menu, NumberInput, Select, Textarea, TextInput } from '@mantine/core';
...
export function Storyboard({ projectId, shots, identity, review, defaultQuality, onRefresh }: {
    ...
    setBusy(false);
} }; const addShots = (placement: ShotPlacement, referenceShotId?: string) => run(() => request(`/projects/${projectId}/shots`, 'POST', { placement, referenceShotId, count: placement === 'before' || placement === 'after' ? 1 : shotInsertCount })); const reviewNow = () => setConfirmation('review'); if (current)
    return <main>
